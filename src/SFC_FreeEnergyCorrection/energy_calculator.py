...
def _validate_linear_system(
    ...
    if ref_idx is not None and 0 <= ref_idx < N:
        ...
        if W is not None:
            if W.ndim == 2 and W.shape[0] == W.shape[1] == P:
                w_orig = np.diag(W)
            elif W.ndim == 1 and W.shape[0] == P:
                w_orig = W
            else:
                raise ValueError("Invalid W shape for augmentation check")
        ...
        if W.ndim == 2 and W.shape[0] == W.shape[1] == P:
            ...
            raise ValueError("Invalid W shape")
    ...
    if cond_A_val > threshold:
        logging.warning(
            f"cond({cond_matrix_name}) = {cond_A_val:.2e} exceeds threshold {threshold:.1e}; "
    ...
    if w_check is not None and col_types == 4:
        ...
        if cond_Aw_val > threshold:
            logging.warning(
                f"cond({cond_weighted_matrix_name}) = {cond_Aw_val:.2e} exceeds threshold {threshold:.1e}; "
...
def _solve_matrix_equations(
    ...
    # --- Augment System ---
    logging.debug(
        f"Augmenting system for matrix solve: ref_idx={ref_idx}, ref_energy={ref_energy:.4f}, ref_weight={ref_weight:.1e}"
    )
    e_ref = np.zeros((1, N), dtype=A.dtype)
    e_ref[0, ref_idx] = 1.0
    A_aug = np.vstack([A, e_ref])
    b_aug = np.concatenate([b, [ref_energy]])
    w_wsfc_aug = None
    if col_types == 4 and W is not None:
        if W.ndim == 2 and W.shape[0] == W.shape[1] == P:
            ...
            logging.warning("WSFC: Invalid W shape")
            col_types = 3
        if col_types == 4:
            w_wsfc_aug = np.concatenate([w_orig, [ref_weight]])
...
def _optimize_loss_function(
    ...
    def loss_function(G, weight_flag):
        ...
        for k, (i, j, adj_ddg, error, _) in enumerate(pairs):
            if i >= N or j >= N:
                raise IndexError(
                    f"Internal error: Molecule index out of bounds ({i}, {j}) for num_mols={N} during loss calculation"
                )
            ...
            if weight_flag is None:  # WSFC: Use exponential decay weight
                ...
                if k >= len(weights_wsfc):
                    raise IndexError(
                        f"Internal error: Pair index {k} out of bounds for weights_wsfc (length {len(weights_wsfc)})"
                    )
                w = weights_wsfc[k]
...
def _irls_optimize_loss_function(
    ...
    for irls_iter in range(irls_max_iter):
        ...
        result = minimize(
            loss_function_irls, G, method="L-BFGS-B", options={"maxiter": max_iter, "gtol": tol, "disp": verbose}
        )
        if not result.success:
        ...
        if verbose:
            print(
                f"IRLS iter {irls_iter}: G_rel_change={G_change:.3e}, W_rel_change={W_change:.3e}, mean_weight={W.mean():.3f}"
            )
...
def _irls_solve_matrix_equations(
    ...
    # --- Augment System ---
    logging.debug(
        f"Augmenting system for IRLS matrix solve: ref_idx={ref_idx}, ref_energy={ref_energy:.4f}, ref_weight={ref_weight:.1e}"
    )
    e_ref = np.zeros((1, N), dtype=A.dtype)
    e_ref[0, ref_idx] = 1.0
    A_aug = np.vstack([A, e_ref])
    ...
    for it in range(irls_max_iter):
        ...
        logging.debug(
            f"IRLS-Matrix iter {it}: G_change={G_change:.3e}, w_change={w_change:.3e}, mean_w={w[:P].mean():.3f}"
        )
        if verbose:
            print(
                f"IRLS-Matrix iter {it}: G_rel_change={G_change:.3e}, w_rel_change={w_change:.3e}, mean_weight(orig)={w[:P].mean():.3f}"
            )

        if G_change < irls_tol and w_change < irls_tol:
            logging.info(f"IRLS-Matrix converged at iteration {it}")
            if verbose:
                print(f"=== IRLS-Matrix converged at iteration {it} ===")
            break
    else:
        logging.warning(f"IRLS-Matrix did not converge within {irls_max_iter} iterations.")
        if verbose:
            print(f"=== IRLS-Matrix Warning: Did not converge in {irls_max_iter} iters ===")

    return {"irls_matrix": G, "irls_matrix_weights": w}
...
def calculate_energies(
    ...
    N = num_mols
    if P == 0:
        logging.warning("Empty pairs list")
        return {}
    if N == 0:
        logging.warning("num_mols is zero")
        return {}
    ...
    for k, pair_data in enumerate(pairs):
        if col_types == 4 and len(pair_data) >= 4:
            i, j, adj_ddg, error = pair_data[:4]
            errors[k] = error
        elif col_types == 3 and len(pair_data) >= 3:
            i, j, adj_ddg = pair_data[:3]
        else:
            raise ValueError(f"Pair data {k} len {len(pair_data)} mismatch {col_types}")
        if not (0 <= i < N and 0 <= j < N):
            raise IndexError(f"Indices {i},{j} out of bounds {N}")
        A[k, i] = -1
        A[k, j] = 1
        b[k] = adj_ddg
    if col_types == 4:
    ...
    try:
        ...
        if mode in ["matrix", "both"]:
            if w_opt_mode == "std_info_weighted":
                ...
                if "sfc_matrix" in matrix_results:
                    results["sfc_matrix_time"] = end_time - start_time
                if "wsfc_matrix" in matrix_results:
                    results["wsfc_matrix_time"] = end_time - start_time  # Using same time for now
                results.update(matrix_results)
            elif w_opt_mode == "irls":
                logging.info("Running IRLS Matrix mode...")
                start_time = time.time()
                irls_matrix_results = _irls_solve_matrix_equations(
                    A, b, N, P, ref_idx, ref_energy, ref_weight, irls_max_iter, irls_tol, verbose
                )
                end_time = time.time()
                ...
                logging.error(f"Unknown w_opt_mode '{w_opt_mode}' for matrix mode.")
        ...
        if mode in ["optimize", "both"]:
            if w_opt_mode == "std_info_weighted":
                ...
                end_time = time.time()
                # Add times for schemes calculated by this function
                if "sfc" in opt_results:
                    results["sfc_time"] = end_time - start_time
                if "wsfc" in opt_results:
                    results["wsfc_time"] = end_time - start_time  # Using same time for now
                results.update(opt_results)
            elif w_opt_mode == "irls":
                logging.info("Running IRLS Optimization mode...")
                start_time = time.time()
                irls_results = _irls_optimize_loss_function(
                    pairs, N, col_types, max_iter, tol, N, irls_max_iter, irls_tol, verbose
                )
                end_time = time.time()
                # Add time for the scheme calculated
                ...
                logging.error(f"Unknown w_opt_mode '{w_opt_mode}' for optimize mode.")
        ...
        if N > 0 and ref_energy is not None and optimization_keys_to_align:
            logging.debug(
                f"Aligning Optimization results to reference energy: {ref_energy} for molecule index {ref_idx}"
            )
                ...
                if scheme in results:
                    ...
                    if isinstance(G_vector, np.ndarray) and G_vector.size == N:
                        if G_vector.size > 0:
                            ...
                            logging.warning(f"Cannot align empty vector for scheme '{scheme}'.")
                    else:
                        logging.warning(f"Result for scheme '{scheme}' is not a valid vector for alignment.")

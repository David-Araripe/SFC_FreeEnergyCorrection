...
def print_pair_results(dl: DataLoader, results: dict, error_results: dict, args):
    ...
    if not filtered_schemes:
        logging.warning(
            "Initial scheme filter in print_pair_results yielded no schemes. Trying a broader filter for ndarrays."
        )
        for k, v in results.items():
        ...
        if not filtered_schemes:
            logging.warning(
                "print_pair_results: No valid energy vector schemes found in results to print after broader filter."
            )
...
def main():
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Show optimization details and write verbose weights CSV"
    )
    parser.add_argument(
        "--max_iter", type=int, default=50000, help="Max iterations for optimizer G-step (default: 50000)"
    )
    parser.add_argument(
        "--tol", type=float, default=1e-8, help="Convergence threshold for optimizer G-step (default: 1e-8)"
    )
    parser.add_argument("--no_header", action="store_true", help="Input file has no header")
    # Mode selects calculation engine(s)
    parser.add_argument(
        "--mode",
        choices=["matrix", "optimize", "both"],
        default="optimize",
    ...
    parser.add_argument(
        "--w_opt_mode",
        choices=["std_info_weighted", "irls"],
        default="std_info_weighted",
    ...
    parser.add_argument(
        "--irls_max_iter", type=int, default=10, help="Max iterations for outer IRLS loop (default: 10)"
    )
    parser.add_argument(
        "--irls_tol", type=float, default=1e-4, help="Convergence threshold for outer IRLS loop (default: 1e-4)"
    )
                ...
                for row_data in node_data_rows:
                    if f"{scheme}_energy" in row_data:
                        ...
                        if not np.isnan(energy_val):
                            f_node.write(
                                f"{row_data['ligand']}\t{energy_val:.4f}\t{path_dep_val:.4f}\t{path_indep_val:.4f}\n"
                            )
            ...
            for key, value in results.items():
                if key.endswith("_weights") and isinstance(value, np.ndarray):
                    ...
                    if not is_augmented and len(weights_vector) != num_orig_pairs:
                        logging.warning(
                            f"Skipping verbose weights for '{scheme_name}': Mismatch between weights ({len(weights_vector)}) and pairs ({num_orig_pairs})"
                        )
                        continue
                    ...
                    try:
                        with open(csv_filename, "w", newline="") as csvfile:
                            ...
                            if is_augmented:
                                writer.writerow(
                                    ["Ref_Constraint", -1, dl.id_map[args.ref], f"{weights_vector[-1]:.6f}"]
                                )
                        ...
                        logging.error(
                            f"Failed to find reference molecule ID '{args.ref}' in id_map for verbose weight output."
                        )
